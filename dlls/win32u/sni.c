...
LRESULT sni_notify_icon( HWND owner, UINT msg, NOTIFYICONDATAW *nid )
{
    ...
    if (msg != NIM_DELETE && (nid->uFlags & NIF_INFO) && nid->szInfo[0])
...
void sni_run_loop( void )
{
    ...
    if (!sni_available || !sni_connection || wake_pipe[0] < 0) return;
    ...
    while (!exit_loop)
    {
        ...
        /* (1) reap dead icons, between dispatch passes, sole closer */
        sni_reap_dead();

        /* (1b) leave when idle so the pump does not spin on an empty list.
        ...
        /* (2) build the poll set: wake-pipe + control conn + each live item conn.
         ...
         * unavailable (poll ignores fd<0), step (6) still drains buffered
         * messages and detects its disconnect. */
        if (!p_dbus_connection_get_unix_fd( sni_connection, &fd )) fd = -1;
        pfds[n].fd = fd; pfds[n].events = POLLIN; svc[n].conn = sni_connection; svc[n].icon = NULL; n++;
        pthread_mutex_lock( &sni_mutex );
        LIST_FOR_EACH_ENTRY( icon, &icon_list, struct sni_icon, entry )
        {
        ...
        /* (3) libdbus can hold whole buffered messages with no new fd activity */
        for (k = 1; k < n; k++)
        ...
        /* (5) wake-pipe readable -> connection set changed. drain and rebuild */
        ...
        /* (6) service control conn first (icon==NULL), then items */
        for (k = 1; k < n; k++)
        {
        ...
        /* (7) release this iteration's snapshot refs */
        for (k = 1; k < n; k++) if (svc[k].icon) icon_release( svc[k].icon );
    }
