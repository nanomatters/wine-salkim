...
/* Recreate after this long with no drained release. Stays above the consumer
 * dmabuf grace window so a briefly paused but alive consumer is not falsely lost. */
...
struct wine_managed_swapchain
{
    ...
    DWORD last_progress_ms;         /* NtGetTickCount of the last drained release (liveness clock) */
...
static VkResult win32u_vkLatencySleepNV(VkDevice device, VkSwapchainKHR swapchain, const VkLatencySleepInfoNV *pSleepInfo)
{
    ...
    if (!vk_swapchain || swapchain_is_out_of_date( vk_swapchain ) || vk_swapchain->managed)
    {
        ...
        if (semaphore)
        {
            ...
            if (!vk_device->p_vkGetSemaphoreCounterValue( vk_device->host.device,
                                          ...
                                          struct wine_managed_swapchain **out )
{
    ...
    *out = NULL;
    ...
    if (create_info->flags || create_info->pNext ||
        create_info->imageArrayLayers != 1 ||
        create_info->imageSharingMode != VK_SHARING_MODE_EXCLUSIVE ||
        create_info->minImageCount > WINE_VK_MANAGED_MAX_IMAGES)
    {
        TRACE( "unsupported managed swapchain contract, falling back to host WSI\n" );
    ...
    managed->alpha_mode = opaque_alpha ? HWND_DMABUF_ALPHA_MODE_IGNORE : HWND_DMABUF_ALPHA_MODE_UNSPECIFIED;
 ...
 * a host queue. There is no host swapchain to do it for us. A binary semaphore
 * is re-signaled only after the consuming present waited on it (deferred release
 * plus WaitIdle leaves it unsignaled). This is always valid. */
...
static VkResult managed_acquire( struct vulkan_device *device, struct swapchain *swapchain,
                                 uint64_t timeout, VkSemaphore host_semaphore, VkFence host_fence,
                                 uint32_t *image_index )
{
    ...
    for (;;)
    {
        ...
        for (i = 0; i < managed->image_count; i++)
        {
            ...
            if (image->valid && !image->acquired && !image->busy)
            {
                slot = idx;
                break;
            }
        ...
        if ((DWORD)(NtGetTickCount() - managed->last_progress_ms) >= WINE_VK_MANAGED_STALL_MS)
        {
            WARN( "managed dmabuf channel stalled, forcing recreate\n" );
            managed_mark_lost( managed );
            pthread_mutex_unlock( &managed->lock );
            return VK_ERROR_OUT_OF_DATE_KHR;
        }
        pfd.fd = managed->channel_fd;
        pthread_mutex_unlock( &managed->lock );

        if (!timeout) return VK_NOT_READY;
        if (timeout != UINT64_MAX)
        {
            uint64_t elapsed_ns, remaining_ns;

            clock_gettime( CLOCK_MONOTONIC, &ts );
            elapsed_ns = ts.tv_sec * 1000000000ull + ts.tv_nsec - start_ns;
            if (elapsed_ns >= timeout) return VK_TIMEOUT;

            remaining_ns = timeout - elapsed_ns;
            if (remaining_ns < poll_timeout * 1000000ull)
                poll_timeout = remaining_ns / 1000000ull +
                               (remaining_ns % 1000000ull != 0);
        }

        if (poll( &pfd, 1, poll_timeout ) < 0 && errno != EINTR)
        {
            pthread_mutex_lock( &managed->lock );
            managed_mark_lost( managed );
            pthread_mutex_unlock( &managed->lock );
...
static void managed_mark_lost( struct wine_managed_swapchain *managed )
{
    ...
    if (managed->channel_fd >= 0)
    {
        close( managed->channel_fd );
        managed->channel_fd = -1;
    }
    managed->lost = TRUE;
    managed->pending_release_token = 0;
    for (i = 0; i < managed->image_count; i++)
    {
                                 ...
                                 BOOL present_waits_consumed )
{
    ...
    BOOL had_channel, send_fd = FALSE;
    RECT client_rect;
    ...
    {
        pthread_mutex_unlock( &managed->lock );
        vulkan_device_lock_queues( device );
        res = device->p_vkDeviceWaitIdle( device->host.device );
        vulkan_device_unlock_queues( device );
        pthread_mutex_lock( &managed->lock );
    }
    if (res < VK_SUCCESS)
    {
    ...
    /* Send the fd until the consumer caches this slot. */
    had_channel = managed->channel_fd >= 0;
    send_fd = had_channel && !image->consumer_cached;
    if (send_fd) channel_fd_dup = dup( image->dmabuf_fd );
    image->acquired = FALSE;
    image->busy = TRUE;
    pthread_mutex_unlock( &managed->lock );

    if (send_fd && channel_fd_dup < 0)
    {
        pthread_mutex_lock( &managed->lock );
    ...
    if (had_channel)
    {
        ...
        else hwnd_dmabuf_post_wake( surface->hwnd );
    }

    /* Keep one old frame in flight. Repeated stalls force recreate. */
    if (had_channel)
    {
        for (;;)
        {
            struct pollfd pfd = { .fd = -1, .events = POLLIN, .revents = 0 };
            BOOL channel_closed, lost;
            uint32_t busy = 0, j;
            int pret;
            ...
            lost = managed->lost;
            channel_closed = managed->channel_fd < 0;
            pfd.fd = managed->channel_fd;
            pthread_mutex_unlock( &managed->lock );

            if (lost) res = VK_ERROR_OUT_OF_DATE_KHR;
            if (busy <= 1 || channel_closed) break;
            ...
            if (pret <= 0)
            {
                ...
                if ((DWORD)(NtGetTickCount() - managed->last_progress_ms) >= WINE_VK_MANAGED_STALL_MS)
                {
                    WARN( "managed dmabuf channel stopped releasing frames\n" );
                    managed_mark_lost( managed );
            ...
            if (pfd.revents & (POLLHUP | POLLERR | POLLNVAL))
            {
                pthread_mutex_lock( &managed->lock );
                managed_drain_releases( managed );
                if (!managed->lost) managed_mark_lost( managed );
                pthread_mutex_unlock( &managed->lock );
                res = VK_ERROR_OUT_OF_DATE_KHR;
                break;
            }
                                             ...
                                             const VkAllocationCallbacks *allocator, VkSwapchainKHR *ret )
{
    ...
    driver_funcs->p_vulkan_surface_set_alpha( create_info->compositeAlpha, surface->client );
    ...
    /* Re-home the client when the Wayland driver changed its presentation
     * topology. The retired host surface remains alive for its old swapchains. */
    for (;;)
    {
    ...
    create_info_host.surface = surface_host_handle( surface );
        ...
        device->extensions.has_VK_EXT_external_memory_dma_buf)
    {
        ...
        hwnd_dmabuf_host_caps_t caps = {0};
        UINT modifier_count = 0;

        if (hwnd_dmabuf_get_caps(surface->hwnd, &caps, NULL, 0, &modifier_count) == HWND_DMABUF_OK &&
            modifier_count)
        {
            ...
            if (res == VK_SUCCESS && managed)
            {
                ...
                *ret = swapchain->obj.client.swapchain;
                TRACE("hwnd %p -> managed cross-process dmabuf swapchain %p\n", surface->hwnd, swapchain);
                pthread_mutex_unlock( &surface->host_lock );
                return VK_SUCCESS;
            }

            TRACE("managed swapchain creation failed, res %d, falling back to host WSI\n", res);
        }
    }
                                            ...
                                            uint64_t presentId, uint64_t timeout )
{
    ...
    if (!client_surface_begin_present_wait( client, swapchain->presentation_generation ))
        return VK_ERROR_OUT_OF_DATE_KHR;
    res = device->p_vkWaitForPresentKHR( device->host.device, swapchain->obj.host.swapchain, presentId, timeout );
...
static VkResult win32u_vkQueuePresentKHR( VkQueue client_queue, const VkPresentInfoKHR *client_present_info )
{
    ...
    uint32_t present_wait_count, host_count = 0;
    ...
    {
        struct swapchain *swapchain = present_swapchains[i] = swapchain_from_handle( present_info->pSwapchains[i] );
    ...
    present_wait_count = present_info->waitSemaphoreCount;
    present_wait_semaphores = host_wait_semaphores;
    ...
    if (blit_count)
    {
        ...
        submit_info.waitSemaphoreCount = present_wait_count;
        submit_info.pWaitSemaphores = present_wait_semaphores;
        ...
        present_wait_count = 1;
        present_wait_semaphores = &blit_sema;
    }
    ...
    if (host_count)
    {
        host_info.waitSemaphoreCount = present_wait_count;
        host_info.pWaitSemaphores = present_wait_semaphores;
        vulkan_queue_lock( queue );
        res = device->p_vkQueuePresentKHR( queue->host.queue, &host_info );
        vulkan_queue_unlock( queue );
        present_waits_submitted = present_result_was_enqueued( res );
        for (uint32_t i = 0; i < host_count; i++)
            present_results[host_indices[i]] = present_waits_submitted ? host_results[i] : res;
    ...
    if (!host_count && !first_managed)
    {
        for (uint32_t i = 0; i < present_info->swapchainCount; i++)
        {
            VkResult cres = present_consume_waits( device, queue, NULL, get_present_fence( present_fence_info, i ),
                                                   i ? NULL : present_wait_semaphores,
                                                   i ? 0 : present_wait_count, FALSE );
            if (cres < VK_SUCCESS)
            {
                res = cres;
                break;
    ...
    if (!host_count && first_managed && present_wait_count)
    {
        VkResult cres = present_consume_waits( device, queue, first_managed, VK_NULL_HANDLE,
                                               present_wait_semaphores, present_wait_count, TRUE );
        if (cres < VK_SUCCESS)
        {
            res = cres;
            skip_managed = TRUE;
    ...
    else if (!host_count && first_managed) present_waits_submitted = TRUE;
    ...
    /* Live entries consume the shared wait array. Stale entries are not sent
     * to that present and therefore need their own fence signal. */
    ...
    if (present_info->pResults)
        for (uint32_t i = 0; i < present_info->swapchainCount; i++)
            present_info->pResults[i] = present_results[i];
