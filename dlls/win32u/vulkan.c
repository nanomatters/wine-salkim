...
static VkResult win32u_vkQueuePresentKHR( VkQueue client_queue, const VkPresentInfoKHR *client_present_info )
{
    ...
    present_wait_semaphores = present_info->pWaitSemaphores;
...
host_present_done:
    ...
    if (!host_count && present_wait_count)
    {
        VkResult cres = managed_present_consume_waits( device, queue, first_managed,
                                                       present_wait_semaphores, present_wait_count );
        if (cres < VK_SUCCESS)
        {
            res = cres;
            skip_managed = TRUE;
