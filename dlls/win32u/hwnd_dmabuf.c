...
/* Send one frame over the channel; drops if full. With dmabuf_fd >= 0 the fd is passed
 * via SCM_RIGHTS (and closed); with dmabuf_fd < 0 only the desc is sent (an fd-less slot
 * reference for a slot the consumer already cached). */
