...
static BOOL check_fsr4_supported(ID3D12Device *device)
{
    ...
    if (SUCCEEDED(ID3D12Device_QueryInterface(device, &IID_ID3D12DeviceExt3, (void **)&ext)))
    {
        BOOL ret = ID3D12DeviceExt3_SupportsAGSExtension(ext, D3D12_AGS_EXTENSION_WMMA_FP8);
