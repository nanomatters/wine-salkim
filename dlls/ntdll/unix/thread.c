 ...
 */
static void start_system_thread( struct thread_data *data )
{
    struct teb_data *thread_data = (struct teb_data *)&data->teb->GdiTebBatch;
    BOOL suspend;
                                      ...
                                      HANDLE process, CLIENT_ID *id, PRTL_THREAD_START_ROUTINE start, void *param )
{
    ...
    InterlockedIncrement( &nb_threads );
    if (pthread_create( &pthread_id, &pthread_attr, (void * (*)(void *))start_system_thread, data ))
    {
        InterlockedDecrement( &nb_threads );
        virtual_free_thread_data( data );
        status = STATUS_NO_MEMORY;
...
{
    struct thread_data *data = get_thread_data();
    NTSTATUS status = send_debug_event( data, rec, context, first_chance, !(is_win64 || is_wow64() || is_old_wow64()) );
