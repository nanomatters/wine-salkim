...
struct thread_data *virtual_alloc_thread_data(void)
{
    ...
    {
        data = view->base;
        set_page_vprot( get_kernel_stack( data ), kernel_stack_guard_size, VPROT_COMMITTED | VPROT_READ );
        mprotect_range( get_kernel_stack( data ), kernel_stack_guard_size, 0, 0 );
        data->request_fd = -1;
        data->reply_fd   = -1;
        data->wait_fd[0] = -1;
        data->wait_fd[1] = -1;
...
NTSTATUS virtual_set_tls_information( PROCESS_TLS_INFORMATION *t )
{
    ...
    server_enter_uninterrupted_section( &virtual_mutex, &sigset );
    LIST_FOR_EACH_ENTRY_REV( data, &teb_list, struct thread_data, entry )
    {
        TEB *teb = data->teb;
