...
static BOOL wayland_hwnd_dmabuf_surface_configure_slices(struct wayland_surface *parent,
                                                         struct wayland_hwnd_dmabuf_surface *surface,
                                                         ...
                                                         struct wl_surface *sibling)
{
    ...
    if (child_width <= 0 || child_height <= 0) return FALSE;
    ...
    if (!(data = get_region_data(parent->child_region))) return FALSE;
    ...
    for (; rect < end; rect++)
    {
        ...
        visible.bottom = min(visible.bottom, dst.bottom);
        if (IsRectEmpty(&visible)) continue;
        if (!(slice = wayland_hwnd_dmabuf_surface_get_slice(surface))) break;
        ...
        wayland_surface_coords_from_window(parent, visible.left, visible.top, &x, &y);
        wayland_surface_coords_from_window(parent, visible.right - visible.left,
                                           visible.bottom - visible.top, &width, &height);
        width = max(1, width);
        height = max(1, height);

        wl_subsurface_set_position(slice->wl_subsurface, x, y);
        wl_subsurface_place_below(slice->wl_subsurface, sibling);
        wp_viewport_set_source(slice->wp_viewport, wl_fixed_from_double(sx),
                               wl_fixed_from_double(sy), wl_fixed_from_double(sw),
                               wl_fixed_from_double(sh));
        wp_viewport_set_destination(slice->wp_viewport, width, height);
                                                  ...
                                                  struct wl_surface *sibling)
{
    ...
    if (wayland_hwnd_dmabuf_surface_configure_slices(parent, surface, info, sibling))
        return TRUE;
