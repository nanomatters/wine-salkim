...
static BOOL wayland_client_surface_set_opaque_region(struct wayland_client_surface *surface,
                                                     BOOL opaque);
static void wayland_client_surface_retarget_image_description(
...
struct wayland_hwnd_dmabuf_buffer
{
    ...
    LONG syncobj_refs; /* prepared syncobj releases retaining this buffer */
    ...
    void *shm_map;     /* writable mapping for host-memory RGBA conversion */
    size_t shm_size;
    BOOL shm_needs_swizzle;
...
enum wayland_hwnd_dmabuf_sync_backend
{
    WAYLAND_HWNDDMABUF_SYNC_IMPLICIT,
    WAYLAND_HWNDDMABUF_SYNC_LEGACY,
    WAYLAND_HWNDDMABUF_SYNC_SYNCOBJ,
};

struct wayland_hwnd_dmabuf_frame_sync
{
    enum wayland_hwnd_dmabuf_sync_backend backend;
    struct wayland_hwnd_dmabuf_buffer *buffer;
    UINT64 acquire_point;
};

struct wayland_hwnd_dmabuf_commit_sync
{
    struct wayland_hwnd_dmabuf_frame_sync *frame;
    struct wayland_syncobj_release *release;
    struct wp_linux_drm_syncobj_surface_v1 **syncobj_surface;
    struct zwp_linux_surface_synchronization_v1 **legacy_surface;
    struct wl_surface *wl_surface;
    int legacy_fd;
};
...
static enum wayland_image_description_status wayland_hwnd_dmabuf_get_color_description(
        ...
        struct wp_image_description_v1 **description)
{
    ...
    *description = NULL;
    if (!state->valid || !process_wayland.wp_color_manager_v1)
        ...
        struct wl_surface *wl_surface, const hwnd_dmabuf_frame_desc_t *desc)
{
    ...
    needs_description = state.valid &&
                        state.color_space != HWND_DMABUF_COLOR_SPACE_SRGB;
    status = wayland_hwnd_dmabuf_get_color_description(&state, &description);
    applied_state = state;
    if (needs_description && status != WAYLAND_IMAGE_DESCRIPTION_READY)
    {
        if (state.color_space == HWND_DMABUF_COLOR_SPACE_SCRGB &&
            status == WAYLAND_IMAGE_DESCRIPTION_UNSUPPORTED &&
            !InterlockedExchange(&warned_unsupported_scrgb, TRUE))
        ...
        if (status == WAYLAND_IMAGE_DESCRIPTION_UNINITIALIZED ||
            ...
            memset(&applied_state, 0, sizeof(applied_state));
    }
    ...
    else if (description)
    {
        if (!surface->management_surface)
            surface->management_surface = wp_color_manager_v1_get_surface(
                    process_wayland.wp_color_manager_v1, surface->wl_surface);
        if (surface->management_surface)
        {
        ...
        {
            ERR("Failed to create color management surface for dmabuf content.\n");
            memset(&applied_state, 0, sizeof(applied_state));
        }
    }
    surface->state = applied_state;
    pthread_mutex_unlock(&surface->mutex);
}
...
/* The owner retires Wayland objects before a syncobj worker can drop the last ref. */
static void wayland_hwnd_dmabuf_buffer_unref(struct wayland_hwnd_dmabuf_buffer *buffer)
{
    ...
    if (buffer->shm_map) munmap(buffer->shm_map, buffer->shm_size);
    ...
    wayland_syncobj_buffer_destroy(buffer->syncobj);
    free(buffer);
}

static BOOL wayland_hwnd_dmabuf_ensure_legacy_sync(
        struct wl_surface *wl_surface,
        struct zwp_linux_surface_synchronization_v1 **explicit_sync)
{
...
static void wayland_hwnd_dmabuf_destroy_syncobj_surface(
        struct wp_linux_drm_syncobj_surface_v1 **syncobj_surface)
{
    if (!*syncobj_surface) return;
        ...
        struct wayland_hwnd_dmabuf_surface *surface, BOOL failed)
{
    ...
    {
        surface->syncobj_failed = TRUE;
        WARN("hwnd=%p disabling linux-drm-syncobj for this presentation surface\n",
             surface->hwnd);
    }
}
...
static void wayland_hwnd_dmabuf_buffer_reap(struct wayland_hwnd_dmabuf_buffer *buffer)
{
    ...
    buffer->surface = NULL;
    wayland_syncobj_buffer_destroy_wayland(buffer->syncobj);
    if (buffer->syncobj && buffer->wl_buffer)
    {
...
/* Release callbacks only update atomic buffer state. */
static void wayland_hwnd_dmabuf_buffer_complete_release(
        struct wayland_hwnd_dmabuf_buffer *buffer)
{
    LONG old, remaining;
...
static void wayland_hwnd_dmabuf_buffer_handle_release(void *data, struct wl_buffer *wl_buffer)
{
    ...
    if (ReadAcquire(&buffer->ignore_wl_release)) return;
    wayland_hwnd_dmabuf_buffer_complete_release(buffer);
}

static void wayland_hwnd_dmabuf_buffer_handle_syncobj_release(void *data, BOOL released)
{
    struct wayland_hwnd_dmabuf_buffer *buffer = data;

    InterlockedDecrement(&buffer->syncobj_refs);
    if (released) wayland_hwnd_dmabuf_buffer_complete_release(buffer);
    else wayland_hwnd_dmabuf_buffer_unref(buffer);
}
...
static BOOL wayland_hwnd_dmabuf_prepare_commit_sync(
        struct wayland_hwnd_dmabuf_frame_sync *frame, struct wl_surface *wl_surface,
        struct wp_linux_drm_syncobj_surface_v1 **syncobj_surface,
        struct zwp_linux_surface_synchronization_v1 **legacy_surface,
        struct wayland_hwnd_dmabuf_commit_sync *sync);
static BOOL wayland_hwnd_dmabuf_stage_commit_sync(
        struct wayland_hwnd_dmabuf_commit_sync *sync);
static void wayland_hwnd_dmabuf_commit_sync(struct wayland_hwnd_dmabuf_commit_sync *sync);
static void wayland_hwnd_dmabuf_cancel_commit_sync(struct wayland_hwnd_dmabuf_commit_sync *sync);
...
static void wayland_hwnd_dmabuf_buffer_prepare_syncobj_ref(
        struct wayland_hwnd_dmabuf_buffer *buffer)
{
    InterlockedIncrement(&buffer->ref);
    InterlockedIncrement(&buffer->syncobj_refs);
}

static void wayland_hwnd_dmabuf_buffer_commit_syncobj_ref(
        struct wayland_hwnd_dmabuf_buffer *buffer)
{
    InterlockedIncrement(&buffer->commit_refs);
    InterlockedExchange(&buffer->ignore_wl_release, TRUE);
}
...
static BOOL wayland_hwnd_dmabuf_surface_attach_slices(struct wayland_hwnd_dmabuf_surface *surface)
{
    ...
    wl_list_for_each(slice, &surface->slices, link)
    {
        struct wayland_hwnd_dmabuf_slice_commit *commit;
    ...
    for (i = 0; i < count; i++)
    {
        ...
        if (commit->slice_buffer)
        {
            commit->slice_buffer->buffer = buffer;
            wayland_hwnd_dmabuf_buffer_add_commit_ref(buffer);
            wl_buffer_add_listener(commit->wl_buffer,
                                   &wayland_hwnd_dmabuf_slice_buffer_listener,
                                   commit->slice_buffer);
            commit->slice_buffer = NULL;
        }
        else
...
static void trigger_window_pos_changed(HWND hwnd)
{
...
static void xdg_popup_handle_done(void *private, struct xdg_popup *xdg_popup)
{
    ...
    trigger_window_pos_changed(hwnd);
    NtUserExposeWindowSurface(hwnd, 0, NULL, 0);
}
...
static const struct wl_buffer_listener wayland_shm_buffer_clone_listener =
{
    wayland_shm_buffer_clone_release
};
                                                             ...
                                                             DWORD exstyle)
{
    ...
    if (ReadAcquire(&client->has_alpha)) return FALSE;
...
static BOOL wayland_hwnd_dmabuf_channel_claimed(HWND hwnd, enum hwnd_dmabuf_status status,
                                                HANDLE handle)
{
    static LONG warned_failure;

    if (status == HWND_DMABUF_OK && handle) return TRUE;
    if (status != HWND_DMABUF_NOT_FOUND && status != HWND_DMABUF_BUSY &&
        !InterlockedCompareExchange(&warned_failure, 1, 0))
        WARN("hwnd=%p channel claim failed, status=%u handle=%p\n", hwnd, status, handle);
    return FALSE;
}

static void wayland_hwnd_dmabuf_surface_claim_channel(struct wayland_hwnd_dmabuf_surface *surface)
{
    static LONG warned_fd_failure;
    enum hwnd_dmabuf_status claim_status;
    NTSTATUS status;
    HANDLE handle = 0;
    int fd = -1;
    ...
    if (surface->gdi_overlay)
        claim_status = wine_hwnd_dmabuf_claim_gdi_overlay_channel(surface->hwnd, &handle);
    else
        claim_status = wine_hwnd_dmabuf_claim_channel(surface->hwnd, &handle);
    if (!wayland_hwnd_dmabuf_channel_claimed(surface->hwnd, claim_status, handle)) return;
    if ((status = wine_server_handle_to_fd(handle, FILE_READ_DATA | FILE_WRITE_DATA, &fd, NULL)))
    {
        if (!InterlockedCompareExchange(&warned_fd_failure, 1, 0))
            WARN("hwnd=%p failed to convert channel handle to fd, status=%#lx\n",
                 surface->hwnd, (long)status);
        fd = -1;
    }
    NtClose(handle);
...
static enum hwnd_dmabuf_channel_result wayland_hwnd_dmabuf_channel_recv_one(
        int channel_fd, hwnd_dmabuf_frame_desc_t *desc, int *out_fd, int *out_sync_fd)
{
    ...
    *out_sync_fd = -1;
    ...
    if (n < 0)
    {
        if (errno == EAGAIN) return HWND_DMABUF_CHANNEL_EMPTY;
#if EWOULDBLOCK != EAGAIN
        if (errno == EWOULDBLOCK) return HWND_DMABUF_CHANNEL_EMPTY;
#endif
        ...
        int fd, BOOL stable_slot)
{
    ...
    buffer->acquire_fd = -1;
    if (desc->flags & HWND_DMABUF_FLAG_SHM_RGBA)
    {
        UINT64 size = desc->offset + (UINT64)desc->stride * desc->height;

        if (desc->stride < (UINT64)desc->width * 4 || size > SIZE_MAX ||
            (buffer->shm_map = mmap(NULL, size, PROT_READ | PROT_WRITE,
    ...
    buffer->cache_valid = stable_slot;
...
static BOOL wayland_hwnd_dmabuf_buffer_enable_wl_release(
        struct wayland_hwnd_dmabuf_buffer *buffer)
{
    struct wl_buffer *wl_buffer;
    int fd;

    if (!ReadAcquire(&buffer->ignore_wl_release)) return TRUE;
    if (InterlockedCompareExchange(&buffer->commit_refs, 0, 0) ||
        InterlockedCompareExchange(&buffer->syncobj_refs, 0, 0))
        return FALSE;
    wayland_syncobj_buffer_destroy_wayland(buffer->syncobj);
    wayland_syncobj_buffer_destroy(buffer->syncobj);
    buffer->syncobj = NULL;
    if ((fd = dup(buffer->data_fd)) < 0) return FALSE;
    if (wayland_hwnd_dmabuf_desc_is_shm(&buffer->desc))
        wl_buffer = wayland_hwnd_dmabuf_create_shm_wl_buffer(buffer->surface, &buffer->desc, fd);
    else
        wl_buffer = wayland_hwnd_dmabuf_create_dmabuf_wl_buffer(buffer->surface, &buffer->desc, fd);
    close(fd);
    if (!wl_buffer) return FALSE;

    if (buffer->wl_buffer) wl_buffer_destroy(buffer->wl_buffer);
    buffer->wl_buffer = wl_buffer;
    InterlockedExchange(&buffer->ignore_wl_release, FALSE);
    wl_buffer_add_listener(buffer->wl_buffer, &wayland_hwnd_dmabuf_buffer_listener, buffer);
    return TRUE;
}
        ...
        struct wayland_hwnd_dmabuf_frame_sync *sync)
{
    ...
    if (!wayland_hwnd_dmabuf_desc_is_shm(&buffer->desc) &&
        wayland_syncobj_available() && !surface->syncobj_failed)
    {
        if ((!buffer->syncobj &&
             !(buffer->syncobj = wayland_syncobj_buffer_create())) ||
            !wayland_syncobj_prepare_acquire(buffer->syncobj, buffer->acquire_fd,
                                             &sync->acquire_point))
        {
            if (buffer->syncobj)
            {
        ...
        {
            sync->backend = WAYLAND_HWNDDMABUF_SYNC_SYNCOBJ;
            return TRUE;
        }
    }

    wayland_hwnd_dmabuf_surface_disable_syncobj(surface, FALSE);
    if (!wayland_hwnd_dmabuf_buffer_enable_wl_release(buffer)) return FALSE;
    if (!wayland_hwnd_dmabuf_desc_is_shm(&buffer->desc) &&
        process_wayland.zwp_linux_explicit_synchronization_v1)
    {
        sync->backend = WAYLAND_HWNDDMABUF_SYNC_LEGACY;
        return TRUE;
    }

    sync->backend = WAYLAND_HWNDDMABUF_SYNC_IMPLICIT;
    if (buffer->acquire_fd < 0) goto prepare_shm;
    if (!wayland_hwnd_dmabuf_desc_is_shm(&buffer->desc) &&
    ...
    if (!wayland_sync_file_wait(buffer->acquire_fd)) return FALSE;

prepare_shm:
    if (buffer->shm_needs_swizzle)
    {
        BYTE *row;
        unsigned int x, y;

        if (!buffer->shm_map) return FALSE;
        row = (BYTE *)buffer->shm_map + buffer->offset;
        for (y = 0; y < buffer->height; y++)
        {
            ...
            {
                uint32_t value = pixel[x];
                pixel[x] = (value & 0xff00ff00u) |
                           ((value & 0x000000ffu) << 16) |
                           ((value & 0x00ff0000u) >> 16);
            }
            row += buffer->stride;
        }
        buffer->shm_needs_swizzle = FALSE;
    }
    return TRUE;
}
...
static BOOL wayland_hwnd_dmabuf_prepare_commit_sync(
        ...
        struct wayland_hwnd_dmabuf_commit_sync *sync)
{
    struct wayland_hwnd_dmabuf_buffer *buffer = frame->buffer;

    memset(sync, 0, sizeof(*sync));
    sync->frame = frame;
    sync->wl_surface = wl_surface;
    sync->syncobj_surface = syncobj_surface;
    sync->legacy_surface = legacy_surface;
    sync->legacy_fd = -1;

    if (frame->backend == WAYLAND_HWNDDMABUF_SYNC_SYNCOBJ)
    {
        if (*legacy_surface)
        {
            zwp_linux_surface_synchronization_v1_destroy(*legacy_surface);
            *legacy_surface = NULL;
        }
        if (!(sync->release = wayland_syncobj_prepare_release(
                buffer->syncobj, wl_surface,
                wayland_hwnd_dmabuf_buffer_handle_syncobj_release, buffer)))
            return FALSE;
        wayland_hwnd_dmabuf_buffer_prepare_syncobj_ref(buffer);
    }
    else if (frame->backend == WAYLAND_HWNDDMABUF_SYNC_LEGACY)
    {
        wayland_hwnd_dmabuf_destroy_syncobj_surface(syncobj_surface);
        if (!wayland_hwnd_dmabuf_ensure_legacy_sync(wl_surface, legacy_surface)) return FALSE;
        if (buffer->acquire_fd >= 0 && (sync->legacy_fd = dup(buffer->acquire_fd)) < 0)
            return FALSE;
    }
    else
        wayland_hwnd_dmabuf_destroy_syncobj_surface(syncobj_surface);
...
static BOOL wayland_hwnd_dmabuf_stage_commit_sync(
        struct wayland_hwnd_dmabuf_commit_sync *sync)
{
    if (sync->frame->backend == WAYLAND_HWNDDMABUF_SYNC_SYNCOBJ)
        return wayland_syncobj_surface_set_points(
                sync->syncobj_surface, sync->wl_surface, sync->frame->buffer->syncobj,
                sync->frame->acquire_point, sync->release);
    if (sync->legacy_fd >= 0)
    {
        zwp_linux_surface_synchronization_v1_set_acquire_fence(
                *sync->legacy_surface, sync->legacy_fd);
        close(sync->legacy_fd);
        sync->legacy_fd = -1;
    }
    return TRUE;
}
...
static void wayland_hwnd_dmabuf_commit_sync(struct wayland_hwnd_dmabuf_commit_sync *sync)
{
    if (!sync->release) return;
    wayland_hwnd_dmabuf_buffer_commit_syncobj_ref(sync->frame->buffer);
    wayland_syncobj_release_commit(sync->release);
    sync->release = NULL;
}

static void wayland_hwnd_dmabuf_cancel_commit_sync(struct wayland_hwnd_dmabuf_commit_sync *sync)
{
    if (sync->legacy_fd >= 0) close(sync->legacy_fd);
    if (sync->release) wayland_syncobj_release_cancel(sync->release);
    sync->legacy_fd = -1;
    sync->release = NULL;
}
        ...
        int acquire_fd)
{
    ...
    buffer->shm_needs_swizzle = !!(desc->flags & HWND_DMABUF_FLAG_SHM_RGBA);
        ...
        BOOL *attached_frame)
{
    ...
    enum hwnd_dmabuf_channel_result channel_result;
    BOOL have_pending = FALSE;
    BOOL retried = FALSE;
    int pfd = -1, psync_fd = -1, fd, sync_fd;
...
retry:
    ...
    while ((channel_result = wayland_hwnd_dmabuf_channel_recv_one(
            surface->channel_fd, &desc, &fd, &sync_fd)) == HWND_DMABUF_CHANNEL_OK)
    {
    ...
    if (channel_result == HWND_DMABUF_CHANNEL_ERROR ||
        channel_result == HWND_DMABUF_CHANNEL_CLOSED)
    {
        if (channel_result == HWND_DMABUF_CHANNEL_ERROR)
            WARN("hwnd=%p channel receive failed, error %d\n", surface->hwnd, errno);
...
commit_current:
    ...
    if (!direct->current_committed)
    {
        ...
        {
            wayland_hwnd_dmabuf_cancel_commit_sync(&commit_sync);
            if (frame_sync.backend == WAYLAND_HWNDDMABUF_SYNC_SYNCOBJ)
                wayland_hwnd_dmabuf_surface_disable_syncobj(direct, TRUE);
...
enum wayland_hwnd_dmabuf_snapshot_result
{
    WAYLAND_HWNDDMABUF_SNAPSHOT_OK,
    WAYLAND_HWNDDMABUF_SNAPSHOT_SERVER_ERROR,
    WAYLAND_HWNDDMABUF_SNAPSHOT_NO_MEMORY,
};

static enum wayland_hwnd_dmabuf_snapshot_result wayland_hwnd_dmabuf_snapshot(
        HWND hwnd, hwnd_dmabuf_frame_info_t **snapshot, unsigned int *snapshot_count,
        enum hwnd_dmabuf_status *server_status)
{
    hwnd_dmabuf_frame_info_t stack_frames[16], *frames;
    unsigned int total = 0, count = 0;
    enum hwnd_dmabuf_status status;

    *snapshot = NULL;
    *snapshot_count = 0;
    *server_status = HWND_DMABUF_OK;
    status = wine_hwnd_dmabuf_list_frames(hwnd, stack_frames, ARRAY_SIZE(stack_frames),
                                          &total, &count);
    if (status != HWND_DMABUF_OK)
    {
        *server_status = status;
        return WAYLAND_HWNDDMABUF_SNAPSHOT_SERVER_ERROR;
    }
    if (!total) return WAYLAND_HWNDDMABUF_SNAPSHOT_OK;

    if (!(frames = calloc(total, sizeof(*frames))))
        return WAYLAND_HWNDDMABUF_SNAPSHOT_NO_MEMORY;
    if (total > count)
    {
        status = wine_hwnd_dmabuf_list_frames(hwnd, frames, total, &total, &count);
        if (status != HWND_DMABUF_OK)
        {
            free(frames);
            *server_status = status;
        ...
        memcpy(frames, stack_frames, count * sizeof(*frames));

    *snapshot = frames;
    *snapshot_count = count;
    return WAYLAND_HWNDDMABUF_SNAPSHOT_OK;
}
...
static BOOL wayland_surface_try_direct_dmabuf(HWND hwnd)
{
    ...
    unsigned int count;
    enum hwnd_dmabuf_status status;
    enum wayland_hwnd_dmabuf_snapshot_result snapshot_result;
    struct wayland_win_data *data;
    ...
    snapshot_result = wayland_hwnd_dmabuf_snapshot(hwnd, &frames, &count, &status);
    if (snapshot_result != WAYLAND_HWNDDMABUF_SNAPSHOT_OK)
    {
        if (snapshot_result == WAYLAND_HWNDDMABUF_SNAPSHOT_NO_MEMORY)
            ERR("hwnd=%p failed to allocate dmabuf frame snapshot\n", hwnd);
        else
...
void wayland_surface_update_hwnd_dmabufs(struct wayland_surface *surface)
{
    ...
    snapshot_result = wayland_hwnd_dmabuf_snapshot(surface->hwnd, &frames, &count, &status);
    if (snapshot_result != WAYLAND_HWNDDMABUF_SNAPSHOT_OK)
    {
        if (snapshot_result == WAYLAND_HWNDDMABUF_SNAPSHOT_NO_MEMORY)
            ERR("hwnd=%p failed to allocate dmabuf frame snapshot\n", surface->hwnd);
    ...
    for (i = 0; i < count; i++)
    {
        ...
        if (configure_result == WAYLAND_HWNDDMABUF_CONFIGURE_UPDATED)
        {
            if (attached_frame && !dmabuf_surface->sliced)
            {
                    ...
                    !wayland_hwnd_dmabuf_stage_commit_sync(&commit_sync))
                {
                    wayland_hwnd_dmabuf_cancel_commit_sync(&commit_sync);
                    if (frame_sync.backend == WAYLAND_HWNDDMABUF_SYNC_SYNCOBJ)
                        wayland_hwnd_dmabuf_surface_disable_syncobj(dmabuf_surface, TRUE);
                    wayland_hwnd_dmabuf_drop_current_frame(
                            dmabuf_surface, HWND_DMABUF_RELEASE_FAILED);
                    wl_surface_attach(dmabuf_surface->wl_surface, NULL, 0, 0);
                    wl_surface_commit(dmabuf_surface->wl_surface);
                    continue;
                }
            ...
            if (!dmabuf_surface->sliced)
                wayland_hwnd_dmabuf_color_surface_sync(
                    &dmabuf_surface->color_surface, dmabuf_surface->wl_surface,
                    &dmabuf_surface->current->desc);
            wayland_hwnd_dmabuf_color_surface_take_pending_commit(
                dmabuf_surface->color_surface);
            if (have_commit_sync) wayland_hwnd_dmabuf_commit_sync(&commit_sync);
            wl_surface_commit(dmabuf_surface->wl_surface);
        ...
        {
            if (!dmabuf_surface->sliced &&
                frame_sync.backend != WAYLAND_HWNDDMABUF_SYNC_SYNCOBJ)
    ...
    {
        wayland_client_surface_stack(surface, data->client_surface,
...
static BOOL wayland_client_surface_is_hwnd_dmabuf_producer(struct wayland_client_surface *surface)
{
    ...
    claim_status = wine_hwnd_dmabuf_claim_channel(hwnd, &handle);
    if (!wayland_hwnd_dmabuf_channel_claimed(hwnd, claim_status, handle)) return FALSE;
...
static BOOL direct_toplevel_enabled(void)
{
    const char *env = getenv("WAYLANDDRV_DIRECT_TOPLEVEL");
                                                            ...
                                                            struct wayland_client_surface *expected_client)
{
    ...
    if (wayland_toplevel_has_visible_child_surface(data->hwnd))
        return "the toplevel has a visible child surface";
                                                   ...
                                                   HWND toplevel)
{
    ...
    presentation_scaled = ReadAcquire(&client->has_presented) &&
                          wayland_client_surface_scales_presentation(surface, client,
                                                                     toplevel_data->content_over_producer);
...
static void wayland_client_surface_reset_image_description_locked(
        ...
        struct wayland_client_surface *surface, struct wl_surface *wl_surface)
{
    ...
    wayland_client_surface_reset_image_description_locked(surface);
    surface->wl_surface = wl_surface;
        ...
        enum wayland_image_description_color_space color_space)
{
    ...
    struct wp_image_description_v1 *image_desc = NULL;

    pthread_mutex_lock(&surface->client.presentation_mutex);
    state.wl_surface = surface->wl_surface;
    if (!state.wl_surface && color_space != WAYLAND_IMAGE_DESCRIPTION_DEFAULT)
    {
        wayland_client_surface_reset_image_description_locked(surface);
        pthread_mutex_unlock(&surface->client.presentation_mutex);
        return FALSE;
    ...
    if (color_space != WAYLAND_IMAGE_DESCRIPTION_DEFAULT &&
        wayland_color_manager_get_image_description(color_space, &image_desc) !=
    ...
    if (image_desc)
    {
        ...
        wp_color_management_surface_v1_set_image_description(
                surface->wp_color_management_surface_v1, image_desc,
                WP_COLOR_MANAGER_V1_RENDER_INTENT_PERCEPTUAL);
    }
    else
        wayland_client_surface_reset_image_description_locked(surface);
